...
export async function refreshNodes() {
  try {
    ...
    if (nodes.length === 0) { show($('nodesList'), '<div class="loading">No nodes</div>'); return; }
    const now = Math.floor(Date.now() / 1000);
    let html = '<table class="cassandra-table"><tr><th>ID</th><th>URL</th><th>Height</th><th>Last Seen</th><th>Status</th></tr>';
    nodes.forEach(n => {
      const isSelf = n.is_self;
      const status = n.is_active === false ? '🔴 Inactive' : '🟢 Active';
      const lastSeen = n.last_seen ? formatTimeAgo(n.last_seen, now) + ' ago' : '—';
      html += `<tr style="${isSelf ? 'opacity:0.7;' : ''}">
        <td class="mono">${escapeHtml((n.node_id||'').slice(0,16))}${isSelf ? ' (self)' : ''}</td>
...
export async function registerNode() {
  ...
  if (!urlRaw) { show($('nodeRegisterResult'), '❌ API URL is required'); return; }
  if (!urlRaw.startsWith('http')) urlRaw = 'http://' + urlRaw;
  let host, port;
  try { const p = new URL(urlRaw); host = p.hostname; port = p.port || '8000'; }
  catch { show($('nodeRegisterResult'), '❌ Invalid URL'); return; }
  try {
